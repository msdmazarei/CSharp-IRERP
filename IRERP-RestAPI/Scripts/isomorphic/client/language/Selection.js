...
cacheSelection: function () {
    ...
    if (this._cachingSelection || this._settingSelected) return;
    ...
    /////////////MSD Changed this line

    /*
    for (var msd in Object.keys(this.data.localData)) {
        i = msd;
    */

    
    if (this.data.Class == 'ResultSet') {
        if (this.data.dataSource.Class == 'RestDataSource') {
            var _msdkeys = Object.keys(this.data.localData);
            for (var msd = 0 ; msd < _msdkeys.length; msd++) {
                var i = parseInt(_msdkeys[msd]);
...
setSelected : function (item, newState, cascadingDirection, recalculate) {
    ...
    if (changed && this.target && this.target.selectionChange) this.target.selectionChange(item, newState);
