    ...
    while (!finished) {
        for (var i = 0; i < this.data.getLength(); i++) { 
            ...
            var startInRange = (eStartTime >= startTime
                && eStartTime < endTime 
                && eStartDay == start.getDay());
                
            var endInRange = (eEndTime > startTime
                && eEndTime <= endTime
                && eEndDay == start.getDay());
...
// work out the size and position of events
    ...
    for (var i = 0; i < events.getLength(); i++) {
        ...
        var eventDay = curr[this.startDateField].getDay();
        ...
        var eventEndDay = curr[this.endDateField].getDay(),
            endHours = curr[this.endDateField].getHours(),
...
_renderEventRange : function (isWeek, start, end) {
    ...
    for (var i = 0; i < localEvents.getLength(); i++) {
        ...
        var periodWidth = colSize / 
                    Math.max(1,colCountArray.max(range.min,range.max-1));
...
isc.DaySchedule.addProperties({
    ...
    getRowHeight : function (record, rowNum) {
        ...
        if (this.creator.scrollToWorkday) {
            var workdayLen = isc.Time.parseInput(this.creator.workdayEnd).getHours() 
    ...
    cellMouseUp : function (record, rowNum, colNum) {
        ...
        if (this.creator.backgroundClick) {
            ...
            if (this.creator.backgroundClick(startDate, endDate) == false) return;
        }
...
isc.TimelineView.addProperties({
    ...
    tagDataForOverlap : function (data, eventType) {
        ...
        if (eventType) {
            while (start < data.getLength() && data.get(start)[cal.eventTypeField] != eventType) {
                start++;
            }
        }
    ...
    sizeEventWindow : function (eventWin, forceRedraw) {        
        ...
        if (styleName) {
           ...
           if (cal.showDescription == false && eventWin.header) {
               eventWin.header.setStyleName(styleName);
           }
        }
