...
isc.Canvas.addClassProperties({
    ...
    _setDoublingStrings : function () {
        this._$noStyleDoublingCSS = isc.Browser.isIE && (!this.neverUseFilters || this.allowExternalFilters) 
                ? "margin:0px;border:0px;padding:0px;background-image:none;background-color:transparent;filter:none;"
...
getPrintInnerHTML : function () {
    var hasChildren = this.children != null && this.children.length > 0;
    var HTML = this._getInnerHTML();
    ...
    if (hasChildren  && HTML == this._$nbsp) return null;
    return HTML;
},
...
_scrollByAmount : function (vertical, amount, reason) {
    if (vertical) {
        ...
        this.scrollTo(this.getScrollLeft() + amount, reason);
    }
},
...
_handleCSSScroll : function (waited, fromFocus) {
    ...
    this.scrollTo(trueScrollLeft, trueScrollTop, "nativeScroll");
...
showRecursively : function () {
    ...
    if (this._containerID == null && parents.isEmpty()) {
        this.show();
    } else {
        this.setVisibility(isc.Canvas.INHERIT);
        if (this._containerID != null) {
            parents.add(window[this._containerID]);
        }
        for (var i = 0; i < parents.length; i++) {
            parents[i].showRecursively();
            if (isc.TabSet != null && isc.isA.TabSet(parents[i])) {
                parents[i].selectTab(parents[i].tabForPane(this));
            } else if (isc.SectionStack != null && isc.isA.SectionStack(parents[i])) {
                parents[i].expandSection(parents[i].sectionForItem(this));
            }
        }
    }
},
...
imgHTML : function (src, width, height, name, extraStuff, imgDir, activeAreaHTML, 
    ...
    if (isc.screenReader) {
        if (extraStuff == null || !isc.contains(extraStuff, "alt=")) {
    ...
    getInnerHTML:function () {
        if (!this._cachedContent) {
            ...
            this._cachedContent = isc.Browser.isIE && isc.Browser.version > 6 ?
                isc.Canvas.imgHTML(this.src, 3200,2400) : isc.Canvas.spacerHTML(3200, 2400);
        }
