...
isc.ValuesManager.addMethods({
    ...
    _updateValue : function (field, value, member) {
        ...
        if (isc.isA.String(field) && field.startsWith(isc.Canvas._$slash)) {
            isDataPath = true;
        } else if (dataPath) {
            field = (field != null) ? this._combineDataPaths(dataPath, field) : dataPath;
            isDataPath = true;
        } else {
            isDataPath = field.contains(isc.Canvas._$slash);
        }
        
        if (!isDataPath) {
            this.values[field] = value;
        } else {
            isc.DynamicForm._saveFieldValue(field, value, this.values, member, true);
        }
        
        if (isDataPath && this.autoSynchronize !== false) {
            var elements = field.split(isc.Canvas._$slash);
            if (parseInt(elements[elements.length-1]) == elements[elements.length-1]) {
                ...
                var fields = this.getFieldsForDataPath(field);
    ...
    validate : function () {
        ...
        for (var fieldName in work) {
            ...
            var fieldObject = work[fieldName],
                ...
                value = isc.DynamicForm._getFieldValue(fieldName, values);
    ...
    setValues : function (values) {
        ...
        var clonedVals = {};
        this._cloneValues(clonedVals, values, this.getDataSource());
        values = clonedVals;
    ...
    rememberValues : function () {
        ...
        this._cloneValues(this._oldValues, values, this.getDataSource(), null, true);
    ...
    _cloneValues : function (storedValues, values, dataSource, dataPath, remembering, dupList) {
        ...
        if (isc.isAn.Array(values)) {
            for (var i = 0; i < values.length; i++) {
                    ...
                    isc.isA.Number(value))
                {
                    ...
                    this._cloneValues(targetObj, value, dataSource, (remembering ? dataPath : null), remembering);
                }
            }
            return;
        }
            ...
            if (isc.isA.Date(propValue)) {
                ...
                if (!field) {
                           ...
                           (field.deepCloneOnEdit == null && deepClone))
                    {
                        ...
                        storedValues[prop] = {};
                        this._cloneValues(storedValues[prop], propValue, 
                            isc.DataSource.get(field.type),
                            remembering ? (fullDataPath + isc.Canvas._$slash) : null,
                            remembering, dupList);
                    } else {
                        storedValues[prop] = values[prop];
                    }
    ...
    getValue : function (fieldName, component) {
        return isc.DynamicForm._getFieldValue(fieldName, this.values, component);
    },
    ...
    setValue : function (fieldName, newValue) {
        ...
        if (!valueSet) {
            if (newValue === undef) {
                ...
                isc.DynamicForm._saveFieldValue(fieldName, newValue, this.values);
            }
        }
    ...
    //
    // @param   member  (Canvas | String) component (or ID of component) to add to 
    ...
    addMember : function (member, fromDataPath) {
        ...
        if (member.dataSource == null && this.dataSource != null && member.getFields) {
            ...
            member.setDataSource(this.dataSource, fields);
        }
    ...
    _setMemberValues : function (member, pickupMemberValues) {
        ...
        var memberDataPath = member.getFullDataPath(),
            ...
            newValues = isc.DynamicForm._getFieldValue(memberDataPath, this.values, member, true),
            multiple = isc.isAn.Array(newValues) || (field && field.multiple),
        ...
        if (multiple) {
            if (member.dataArity == "single") {
                ...
                if (selComponent != null) {
                    //return;
                    newValues = newValues[selComponent.getRecordIndex(selComponent.getSelectedRecord())];
                } else {
        ...
        if (!isc.isA.DynamicForm(member)) {
            ...
            if (newValues == null) {
                // FIXME: This is clearly wrong - this method call is missing a param and will
                //        no-op - but I'm not sure what it should be doing right now...
            ...
            for (var i = 0; i < items.getLength(); i++) {
                ...
                // Do NOT apply the member dataPath to a field that specifies an absolute path
                // (one that starts with a slash)
                if (memberDataPath && !fullFieldPath.startsWith(isc.Canvas._$slash)) {
                ...
                var currentFieldValue = isc.DynamicForm._getFieldValue(fullFieldPath, 
                                                                       member.values, 
                                                                       member, true);
                var newFieldValue = isc.DynamicForm._getFieldValue(fullFieldPath, 
                                                                   this.values, 
                                                                   member, true);
                ...
                if (currentFieldValue !== undef) {
                     if (newFieldValue !== undef) {
                        ...
                        member.setValue(fullFieldPath, newFieldValue);
                    } else {
                        // explicitly calling 'clearValues()' will cause dynamic defaults to be
                        // re-evaluated
                        if (!pickupMemberValues) member.clearValue(fullFieldPath);
                    }
                } else if (newFieldValue !== undef) {
                    member.setValue(fullFieldPath, newFieldValue);
                }
                ...
                var newFieldVal = member.getValue(fullFieldPath);
