...
//	unknownErrorMessage : "Invalid value",
    ...
    // If set to false, the form will be marked read-only. A widget on the form is only
    // considered editable if it is not individually marked read-only and all parents
    // above it in the containment hierarchy, including the form, are also not marked
    // read-only. This setting allows you to enable or disable editability of the form
    // and all components at one time.
    // <P>
    ...
    // <P>
    // Note that a form and formItem are considered editable if canEdit is null (default) or
    // <code>true</code>.
    //
    // @setter setCanEdit()
    // @group readOnly
...
initWidget : function () {
    ...
    // If we have a valuesManager and it is a string, assume it is a global ID
    if (isc.isA.String(this.valuesManager)) this.valuesManager = window[this.valuesManager];
},
...
rememberValues : function () {
    ...
    // dataPaths: If we have items with a specified dataPath we're going to end up with 
    // nested objects.
    // These objects will get modified in place when the user changes the form item values, so we
    // have to basically clone them onto the _oldValues object.
    // Rather than calling clone() on every property we hit that's an object [dangerous - won't handle
    // circular references, may attempt to serialize out huge data structures, etc], explicitly catch
    // these cases, and duplicate the properties referenced in the items' dataPaths
    var dataPaths = [],
        dataPathRoots = {},
        items = this.getItems() || [];
    for (var i = 0; i< items.length; i++) {
        if (items[i].dataPath != null) {
            dataPaths[dataPaths.length] = items[i].dataPath.split("/");
        }
    }
    
    if (dataPaths.length > 0) {
        for (var i = 0; i < dataPaths.length; i++) {
            var currentSourceObject = values, 
                currentTargetObject = newValues,
                dataPath = dataPaths[i];
                
            // note that we're handling this nested object, so we don't pick it up in
            // our simple copy below.
            dataPathRoots[dataPath[0]] = true;
                
            for (var ii = 0; ii < dataPath.length; ii++) {
                var segment = dataPath[ii];
                if (currentSourceObject[segment] != null) {
                    var sourceValue = currentSourceObject[segment],
                        targetValue = currentTargetObject[segment],
                        isObject = isc.isAn.Object(sourceValue);
                        
                    if (isObject && (targetValue == null || targetValue == sourceValue)) {
                        if (isc.isA.Date(targetValue)) {
                            currentTargetObject[segment] = new Date(targetValue.getTime());
                        } else if (isc.isAn.Array(targetValue)) {
                            currentTargetObject[segment] = targetValue.duplicate();
                        } else {
    
                            currentTargetObject[segment] = isc.addProperties({}, sourceValue);
                       } 
                    } else if (targetValue == null) {
                        currentTargetObject[segment] = sourceValue;
                    }
                    
                    currentSourceObject = currentSourceObject[segment];
                    currentTargetObject = currentTargetObject[segment];
                } else {
                    break;
                }
            }
	...
	for (var prop in values) {
        ...
        // Skip any objects handled by nested datapath values
        if (dataPathRoots[prop] == true) continue;
        // Dates are modified in place as are Objects and arrays [likely managed by
        // dataPath type functionality].
        // Duplicate these into _oldValues so we can check for changes / restore to previous values.
...
setValuesAsCriteria : function (criteria, advanced) {
    ...
    // We'll update values item-by-item via 'setCriterion()' or for simple criteria
    // item.setValue()
    this.setValues({});
    if (!advanced && !isc.DataSource.isAdvancedCriteria(criteria)) {
        ...
        for (var i = 0; i < items.length; i++) {
            for (var fieldName in criteria) {
                if (items[i].canEditSimpleCriterion(fieldName)) {
                    items[i].setSimpleCriterion(criteria[fieldName], fieldName);
                    break;
                }
            }
        }
...
getItem : function (itemName) {
    ...
    for (var i = 0; i < this.items.length; i++) {
        ...
        if (path == itemName) return this.items[i];
        // Look for a dataPath expressed relative to this form's dataPath
        if (this.dataPath) {
            if (this.dataPath.endsWith("/")) {
                if (this.dataPath + path == itemName) return this.items[i];
            } else if (this.dataPath + "/" + path == itemName) return this.items[i];
        }
    }
...
saveItemValue : function (item, value) {
    ...
    var	dataFieldID = isc.DynamicForm._combineDataPaths(this.dataPath, item.getDataPath() || 
                                                                       item.getFieldName());
    if (dataFieldID != null) {
...
_saveValue : function (field, value) {
    var fieldName;
    if (isc.ValuesManager && isc.isA.ValuesManager(this.valuesManager)) {
        var fieldObj = this.valuesManager.getField(field);
        if (fieldObj) fieldName = fieldObj.name;
    }
    isc.DynamicForm._saveFieldValue(field, value, this.values, this, true, false, fieldName);
...
setItemValues : function (values, onRedraw, initTime, items) {
	...
	for (var itemNum = 0; itemNum < items.length; itemNum++) {
        ...
        //    if (nestedValues != null) value = nestedValues[segments.last()];
            value = isc.DynamicForm._getFieldValue(dataPath, values, this, true);
...
validate : function (validateHiddenFields, ignoreDSFields, typeValidationsOnly, checkValuesOnly, skipServerValidation) {
    ...
    for (var itemNum = 0; itemNum < this.items.length; itemNum++) {
    	var fieldErrorsFound = false,
            ...
            dp = isc.DynamicForm._combineDataPaths(this.dataPath, item.getDataPath()),
        ...
        if (item.validators != null) {
            ...
            if (fieldResult != null) {
                ...
                if (fieldResult.resultingValue != null) { 
                    ...
                    if (dp) {
                        isc.DynamicForm._saveFieldValue(dp, value, values);
                    } else if (column) {
                        values[column] = value;
                    }
                    hasChanges = true;
...
focusAtEnd : function (start) {
    ...
    if (startItem && this._canFocusInItem(startItem, true)) this.focusInItem(startItem, !!start);
...
handleKeyDown : function (event,eventInfo) {
    ...
    if (keyDownReturn != false && this._formItemKeys[keyName]) {
...
_getHoverProperties : function (item) {
    ...
    if (item) {
        props = isc.addProperties({}, {
            ...
            align:(item.hoverAlign != null ? item.hoverAlign : this.itemHoverVAlign),
...
// form items to render as read-only.
//
...
getEditorType : function (field, widget) {
    ...
    var canEdit = this.canEditField(field,widget),
        ...
        type = (canEdit == false && field.readOnlyEditorType) 
               || field.editorType || field.formItemType || field._constructor ||
               field.type || defaultType;

    

    if ((canEdit == false && field.readOnlyEditorType) || field.editorType || 
...
canEditField : function (field, widget) {
    ...
    if (widget && widget.canEditField) return widget.canEditField();
    
    var fieldCanEditAttribute = (widget && widget.canEditFieldAttribute) || "canEdit";
    if (field[fieldCanEditAttribute]!= null) return field[fieldCanEditAttribute];
...
compareValues : function (value1, value2) {
    ...
    // These array / date comparisons are not recurive so won't compare nested properties - just
    // simple equality one level deep. Probably ok.
...
valuesHaveChanged : function (form, returnChangedVals, values, oldValues, rootPath) {
    ...
    for (var prop in values) {
        ...
        if (prop == this._$refPropName) continue;
