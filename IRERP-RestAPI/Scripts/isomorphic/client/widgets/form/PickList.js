...
isc.PickList.addInterfaceProperties({
    ...
    // <P>
    // NOTE: When enabled, +link{getSelectedRecord,getSelectedRecord} will only include the
    // fetched fields.
    // @visibility external
    //<
...
isc.PickList.addInterfaceMethods({
    ...
    fetchData : function (callback, requestProperties, maintainCache) {
        ...
        if (!this.pickList) {
            ...
            this.filterDataBoundPickList(requestProperties, !maintainCache);
        }
    },
   ...
   setUpPickList : function (show, queueFetches, request, dropCache) {
        ...
        this.filterPickList(show, queueFetches, request, dropCache);
    }, 
    ...
    _translateValueFieldValue : function (value, toValueField, returnNull) {
        ...
        var resultSet = this.pickList && !this.pickList.destroyed ? 
    ...
    filterDataBoundPickList : function (requestProperties, dropCache) {
        ...
        if (synchronousFilter && this.pickList.data.getLength() > 0 && 
            !this.pickList.data.rowIsLoaded(0)) 
        {
    ...
    placePickList : function () {
        ...
        // so placeNear does the right thing.
