...
// +link{DateItem} controls in a form, using +link{FormItem.operator} and
...
// the standard +link{dateFormatter} and +link{inputFormat} and 
...
getCriterion : function (absolute) {
    ...
    if (hasFromValue || hasToValue) {
        ...
        if (hasFromValue) {
            if (isc.isA.RelativeDateItem(fromValue)) {
        ...
        if (hasToValue) {
            if (isc.isA.RelativeDateItem(toValue)) {
...
//> @attr dateRangeItem.dateRangeForm (AutoChild : null : R)
...
isc.DateRangeItem.addMethods({
    ...
    _createEditor: function () {
        ...
        var titleOrientation = this.innerTitleOrientation || this.titleOrientation || this.form.titleOrientation;
...
// Button used for clearing the dialog values.  Note that, since this is an AutoChild, it can be 
...
// Mini DateRangeItems are marked as canFocus:true, and set up with properties such that focus
