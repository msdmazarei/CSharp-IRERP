...
fieldKeyDefaults: {_constructor: "ListGrid",
    ...
    defaultFields: [
        ...
        {name: "title"},
        {name: "sourceDS", showIf: "list.creator.dataSources != null"},
    ...
    // Note - this isn't actually ideal UE - to use it you'd have to click on a field,
    // enter "+" (say), then click on the next field
    recordClick : function (viewer, record) {
        ...
        if (formulaField) {
            ...
            if (selectionRange != null) {
                value = value.substring(0,selectionRange[0]) + record.mappingKey + value.substring(selectionRange[1]);
            } else {
...
// The default title for the "Source DataSource" column in the +link{fieldList}. Only shown
// if +link{formulaBuilder.dataSources} is used to specify multiple dataSources.
// @group i18nMessages
// @visibility rules
//<
...
getAvailableFields : function (fields, currentField) {
    ...
    for (var i = 0; i < fields.getLength(); i++) {
        ...
        if (currentField && currentField.name == item.name) continue;
    ...
    if (!record && fields) {
        ...
        for (var i = 0; i < fields.length; i++) {
            ...
            if (item._generatedFormulaFunc) {
                ...
                isc.DataSource.setPathValue(record, item[fieldIdProperty], 
                ...
                {
                    isc.DataSource.setPathValue(record, item[fieldIdProperty], 1);
                } else {
                    isc.DataSource.setPathValue(record, item[fieldIdProperty], item[fieldIdProperty]);
                }
            else {
                isc.DataSource.setPathValue(record, item[fieldIdProperty], item[fieldIdProperty]);
           }
...
generateFunction : function (userFormula, fields, component, allowNonNumeric) {
	var output = isc.SB.create(),
        ...
        if (usedFields.length > 0) {
            ...
            for (var i = 0; i < usedFields.length; i++) {
                ...
                output.append(
                    ...
                    "\nif(", item.mappingKey, "==null)", 
                    item.mappingKey,
                    "= (component ? component.getStandaloneFieldValue(record, '",fieldName,
                            "', true) : " + (allowNonNumeric ? "null)" : "0)")
                );
