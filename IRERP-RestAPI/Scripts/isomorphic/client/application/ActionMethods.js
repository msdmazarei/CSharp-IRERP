...
isc.EditorActionMethods.addInterfaceMethods({
    ...
    // <i>even if willHandleError:true</code> was specified on the dsRequest - though for other
    // error codes, the callback would be fired if willHandle error is specified on the request.
    ...
    saveData : function (callback, requestProperties, noValidation) {
        ...
        if (isc.ValuesManager && isc.isA.ValuesManager(this.valuesManager) && this.dataPath) {
            ...
            for (var i = 0; i < fields.length; i++) {
                var fieldId = fields[i].dataPath || fields[i].name;
        ...
        if (!this.suppressServerDataSync && response && response.status >= 0 && data != null) {
            ...
            if (hasChanges) {
                ...
                for (var i = 0; i < this.items.length; i++) {
                    ...
                    var submittedVal = isc.DynamicForm._getFieldValue(path, submittedValues),
                        serverVal = isc.DynamicForm._getFieldValue(path, data);
