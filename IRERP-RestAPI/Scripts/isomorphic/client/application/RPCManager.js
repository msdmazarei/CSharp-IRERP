                ...
                if (operation && operation.isRestRequest && transaction.operations.length > 1) {
                    ...
                    if (operation.dataFormat == "json") {
                        ...
                        this.logWarn("Found a REST request that appears to be in JSON format, " +
                            "but the response was not wrapped in start/end markers - aborting")
                        return;
                    } else {
		...
		for (var i = 0, j = 0; i < requests.length; i++) {
            ...
            if (responseIsStructured && request.clientOnly) {
                ...
                response = isc.addProperties(this.createRPCResponse(transaction, request), {
                    ...
                    data : responseIsStructured && request.isRestRequest ? results[j] : null
                });
                j++;
            }
...
//<
   ...
   // <P>
   // Also you can use a custom skin for export your HTML content. To use your custom skin 
   // you have to add a new line into server.properties file, that line will be:
   // <P>
   // skin.{skinName}.location: custom/skin
   // <P>
   // Where {skinName} is the name of your custom skin, and the root of your directories must 
   // be located in the webroot of your application.
   // <P>
   // Requires the SmartClient server framework, but does not require use of server-based
   // databinding.
   //
   // @param canvas (Canvas | HTMLString) Canvas that has exportable widgets, or HTML fragment
   //                                     derived from
   ...
   exportContent  : function (canvas, requestProperties) {
        var callback = function (html) {
            var props = requestProperties || {},
                skinName = props && props.skinName ? props.skinName : "skin_styles",
                pdfName = props && props.pdfName ? props.pdfName : "export"
	        ;          
            var serverProps = {
                showPrompt:false,
            ...
            var settings = {
                ...
                dirDefaultSkin: isc.Page.getSkinDir()//get default skin directory
            };
            isc.DMI.callBuiltin({
        ...
        var HTML = canvas.getPrintHTML(null, callback);
        if (HTML != null) {
            callback(HTML);
        }
    }
...
//           Be sure to read the overview of +link{group:sqlVsJPA,SQLDataSource vs
//           JPA/Hibernate} in order to understand the large benefits the SQLDataSource provides</li>
