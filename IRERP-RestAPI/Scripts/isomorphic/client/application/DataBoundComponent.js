...
_performActionOnValue : function(action, fieldName, values, component, 
                                 useFirstEntryImplicitly, value, skipExtraCheck)
{
    ...
    if (isDataPath) {
        fieldName = fieldName.trim(isc.Canvas._$slash);
			...
			action == "get" && !skipExtraCheck) 
        {
            ...
            {
                return this._performActionOnValue(action, fieldName, 
            ...
            if (i == segments.length-1) {
                ...
                if (isc.isAn.Array(values)) {
                    ...
                    // selectionComponents
                        ...
                        var path = isc.Canvas._$slash,
                            selComponent = component,
                            foundNullSelection;
                        for (var j = 0; j <= i; j++) {
                            path += segments[j] + isc.Canvas._$slash;
                        }
                        path = path.trim(isc.Canvas._$slash);
                        selComponent = component.selectionComponent;
                        while (selComponent) {
                            var componentPath = selComponent.dataPath;
                            if (componentPath) componentPath = componentPath.trim(isc.Canvas._$slash);
                            if (path == componentPath) {
                                var sel = selComponent.getSelectedRecord();
                                if (sel) {
                                    index = selComponent.getRecordIndex(sel);
                                } else {
        ...
        if (action == "get") return values[fieldName];
        else if (action == "clear") delete values[fieldName];
        else if (action == "save") values[fieldName] = value;
    }
},
...
{
    this._performActionOnValue("clear", field, values, component, useFirstEntryImplicitly,
								null, false); 
},
...
_saveFieldValue : function (field, value, values, component, useFirstEntryImplicitly)

{
    this._performActionOnValue("save", field, values, component, useFirstEntryImplicitly, 
                               value, false);
    return values;
},
...
_getFieldValue : function (field, values, component, useFirstEntryImplicitly) 
{
    return this._performActionOnValue("get", field, values, component, useFirstEntryImplicitly,
									  null, false);
},
    ...
    if (dataPathComponent) {
        if (dataPathComponent != this || !this.dataSource) {
            ...
            if (dataPathComponent.valuesManager.getDataSource()) {
                this.setDataSource(dataPathComponent.valuesManager.getDataSource(), fields);
            }
...
buildFieldDataPath : function (componentDataPath, field) {
    ...
    if (componentDataPath && dataPath && !dataPath.startsWith("/")) {
        dataPath = componentDataPath + "/" + dataPath;
    }
...
getFieldState : function (includeTitle) {
    ...
    if (allFields) {
        for (var i = 0; i < allFields.length; i++) {
            ...
            if (field.excludeFromState) continue;
...
_setFieldState : function (fieldState, hideExtraDSFields) {
    ...
    for (var i = 0; i < fieldState.length; i++) {
        ...
        if (state.autoFitWidth != field.autoFitWidth) field.autoFitWidth = state.autoFitWidth;
...
exportData : function (requestProperties, callback) {
    ...
    this.getDataSource().exportData(this.getCriteria(), requestProperties, callback);
},
...
useExistingDataModel : function (criteria, operation, context) {
    ...
    if (!(isc.isA.ResultSet(resultSet) || isc.isA.ResultTree(resultSet))) {
        resultSet = this.originalData;
        if (resultSet == null) return false;
        if (!isc.isA.ResultSet(resultSet) && !isc.isA.ResultTree(resultSet)) return false;
    }
...
updateDataModel : function (filterCriteria, operation, context) {
    ...
    if (!isc.isA.ResultSet(resultSet)) resultSet = this.originalData;
    
    if (!isc.isA.ResultSet(resultSet)) {
        return;
    }
...
evaluateCriterion : function (record, criterion) {
    ...
    var ds = this.getDataSource();
    return op.condition(criterion.value, record, criterion.fieldName, criterion, op, ds || this);
},
...
getCleanRecordData : function (record) {
    ...
    for (var key in record) {
        ...
        if (key == "_recordComponents") continue;
        if (key == "_embeddedComponents") continue;
...
exportClientDataReply : function (data, context) {
    ...
    var props = context || {},
        format = props.exportAs ? props.exportAs : "csv",
        fileName = props.exportFilename ? props.exportFilename : "export",
        exportDisplay = props.exportDisplay ? props.exportDisplay : "download"
    ;
    ...
    if (props.exportFields) {
        ...
        // if exportFields was specified, map the provided fieldNames to the relevant titles,
        // since we don't know anything about REAL fieldnames when exporting client-data...

        for (var i=0; i<props.exportFields.length; i++) {
            var fieldName = props.exportFields[i],
                field = allFields.find("name", fieldName),
                fieldTitle = field ? (field.exportTitle ? field.exportTitle : field.title) : null
            ;
            ...
            if (fieldTitle) {
                ...
                if (fieldTitle != escapedTitle && data && data.length) {
                	for (var j=0; j<data.length; j++) {
                		data[j][escapedTitle] = data[j][fieldTitle];
                		delete data[j][fieldTitle];
                	}
                }
                exportFields.add(escapedTitle);
			} else {
        ...
        if (missingFields.length > 0) {
            this.logWarn("exportFields was specified but contains the following field-names "+
                "that are not available in this component: "+missingFields.join(",")+".");
        }
        
        if (exportFields.length > 0) settings.exportFields = exportFields;
    }
